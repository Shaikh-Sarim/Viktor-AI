        ...
        <div className="max-w-7xl mx-auto px-4 py-20">
          ...
          <div className="text-center mb-20">
            <div className="mb-4 inline-block px-4 py-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/50 rounded-full backdrop-blur">
              <span className="text-sm font-semibold text-cyan-300">Choose Your Plan</span>
            </div>
            ...
            <p className="text-lg text-slate-300 mb-3">
              Hello, <span className="font-semibold text-cyan-300">{session.user?.email}</span>
            </p>
            <p className="text-slate-400 text-lg">
              Select your plan to unlock AI-powered content generation locally
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            {plans.map((plan) => (
              <div
                ...
                className={`cursor-pointer rounded-2xl transition-all duration-300 transform p-8 relative group ${
              ...
              >
                {plan.recommended && (
                  <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 animate-bounce">
                    <span className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-lg">
                      ⭐ MOST POPULAR
                    </span>
                  </div>
                )}
                ...
                <div className="mb-8">
                  ...
                  <p className={`text-sm ${selectedPlan === plan.id ? "text-cyan-200" : "text-slate-400"}`}>
                    {plan.description}
                  </p>
                </div>
                ...
                <div className="mb-8 pb-6 border-b border-slate-700/50">
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className={`text-4xl font-bold ${selectedPlan === plan.id ? "text-white" : "text-cyan-400"}`}>
                      {plan.price}
                    </span>
                    {plan.price !== "$0" && (
                      <span className={`text-sm ${selectedPlan === plan.id ? "text-cyan-200" : "text-slate-400"}`}>
                        /month
                      </span>
                    )}
                  </div>
                  <div className={`text-sm font-semibold ${selectedPlan === plan.id ? "text-cyan-100" : "text-cyan-400"}`}>
                ...
                <div className="space-y-4 mb-8">
                  {plan.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      <div className={`w-5 h-5 mt-0.5 flex-shrink-0 rounded-full flex items-center justify-center ${selectedPlan === plan.id ? "bg-cyan-400" : "bg-green-500"}`}>
                        <span className="text-xs font-bold text-slate-900">✓</span>
                      </div>
                      <span className={`text-sm leading-relaxed ${selectedPlan === plan.id ? "text-cyan-50" : "text-slate-300"}`}>
                        {feature}
                      </span>
                    </div>
                  ))}
          ...
          {/* Info Banner */}
          <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/30 rounded-2xl p-8 mb-12 backdrop-blur">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-slate-300">
                  Upgrade, downgrade, or cancel your plan anytime. Changes take effect immediately on your account.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center">
            <button
              ...
              disabled={loading}
              className="px-12 py-4 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold rounded-xl text-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(34,211,238,0.5)] hover:shadow-[0_0_30px_rgba(34,211,238,0.7)] transform hover:scale-105"
            >
              {loading ? (
                ...
                `${selectedPlanObj?.isPaid ? "Proceed to Payment" : "Start Free"} • ${selectedPlanObj?.name}`
              )}
            </button>
